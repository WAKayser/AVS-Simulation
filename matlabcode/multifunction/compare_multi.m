function [detection, falsePos]=compare_multi(avsdata, eventdata, eventVec, peakVector, param)
% this function is uesed to compare the data that multple dspmodules generate    
    ...
    for j = 1:size(avsdata, 3)
        for i = 1:size(avsdata, 2)
            ...
            [detection(:,i,j), falsePos(i,j)] = compare(avsdata(:,i,j), eventdata, eventVec(:,i,j), peakVector, param);
        end
    end
end
